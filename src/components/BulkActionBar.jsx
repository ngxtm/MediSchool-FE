...
const BulkActionBar = ({
		...
		<Menu>
			{templates.map(tpl => (
				<Menu.Item key={tpl.id} onClick={() => onBulkTemplate(tpl)}>
					<div>
						<div className={`font-medium ${tpl.isAbnormal ? 'text-red-600' : 'text-green-600'}`}>
							{tpl.name}
						</div>
	...
	return (
		<div
			className={`${styles.bg} ${styles.border} border rounded-lg p-4 mb-4 flex items-center justify-between shadow-sm`}
		>
			<div className="flex items-center gap-3">
				...
				<span className={`font-medium ${styles.text}`}>
					{isVaccineMode
						...
						: `Đã chọn ${selectedCount} học sinh chưa phản hồi`}
				</span>
			</div>
