...
export const formatCreateDate = date => {
  if (!date) return ''
  return typeof date === 'number' ? dayjs.unix(Math.floor(date)).format('DD/MM/YYYY') : dayjs(date).format('DD/MM/YYYY')
}
  ...
  const vacc_columns = [
    {
      ...
      width: 180,
      render: text => {
        return <span className="font-inter font-semibold">{text}</span>
      },
    ...
    },
    {
      ...
      width: 180,
      render: (_, record) => {
        return <span className="font-inter font-semibold">{record.vaccine.name}</span>
      },
      onHeaderCell: () => ({ style: { fontFamily: 'var(--font-inter)', color: '#115e59' } })
    },
    {
    ...
    },
    {
      ...
      width: 180,
      render: (_, record) => {
        return <span className="font-inter font-semibold"> {record.createdBy?.fullName || 'Chưa có'}</span>
      },
      onHeaderCell: () => ({ style: { fontFamily: 'var(--font-inter)', color: '#115e59' } })
    },
    {
        ...
        return (
          <div className="flex flex-col gap-2">
            <button
              ...
              className="cursor-pointer rounded-lg bg-teal-600 px-4 py-0.5 font-semibold text-white transition-colors duration-200 hover:bg-teal-700"
            >
              Duyệt
            </button>
            <button
              onClick={() => handleRejectClick('vaccine', record)}
              className="cursor-pointer rounded-lg border-1 border-teal-700 bg-white px-4 py-0.5 font-semibold text-teal-700 transition-colors duration-200 hover:bg-teal-200"
            >
              Từ chối
            </button>
          </div>
  ...
  return (
    <div className="font-inter">
      ...
      <div className="mt-12 flex items-center gap-2">
        <p className="rounded-lg bg-gradient-to-r from-emerald-500 to-lime-600 px-6 py-2 text-xl font-bold text-white">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2">
          <p className="rounded-lg bg-gradient-to-r from-teal-500 to-emerald-500 px-6 py-2 text-xl font-bold text-white">
            <span className="flex items-center gap-5">
              <Stethoscope size={28} />
      ...
      <div className="mt-12 flex items-center gap-2">
        <p className="rounded-lg bg-gradient-to-r from-teal-500 to-cyan-600 px-6 py-2 text-xl font-bold text-white">
          <span className="flex items-center gap-5">
            <Package size={28} />
