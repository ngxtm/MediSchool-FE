		...
		<>
			...
			<div className="flex justify-between">
				...
				<div className="flex gap-10">
					<button
						className="border-1 border-teal-300 text-teal-600 hover:bg-teal-50 bg-transparent  px-7 py-1.5 rounded-lg font-bold text-base transition-all duration-200 ease-in-out hover:scale-105 hover:shadow-lg active:scale-95"
					...
					sortedEvents.map((event) => {
						...
						return (
							<Link to={`vaccine-event/${event.id}`}>
								<div
									key={event.id}
									className="flex rounded-xl bg-white hover:shadow-xl w-full justify-between max-w-[80rem] mx-auto border-gray-300 border-b-1 border-t-1 p-6 transition-colors hover:bg-gray-50 cursor-pointer group"
								>
									...
									<div className="flex items-center gap-10">
										<div className="flex flex-col gap-2 min-w-[200px] items-center">
											{statusText === "Đã hủy" ? (
												<>
													<p
														className={`text-center border-1 ${borderColor} ${bgColor} ${textColor} font-bold px-9 py-1 w-fit rounded-4xl`}
													>
														{statusText}
													</p>
												</>
										...
										<button>
											<ChevronRight
												size={30}
												className="transition-transform duration-200 group-hover:translate-x-1 group-hover:scale-110 text-teal-600"
											/>
										</button>
									</div>
								</div>
